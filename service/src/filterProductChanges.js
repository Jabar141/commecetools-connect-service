const { fetchCurrentAndStagedProductDetails } = require('./productQuery');
...
module.exports.reviewProductChanges = async(productID) => {
...
async function formatNameChanges(current, staged, locales) {
...
async function formatDescChanges(current, staged, locales) {
...
async function formatMasterVariantChanges(current, staged, locales) {
    ...
    const currMasterVarAttributeName = new Set(await Promise.all(currentMasterVariantAttributes.map(async (attr) => { return attr.name; })));
    const stgMasterVarAttributeName = new Set(await Promise.all(stagedMasterVariantAttributes.map(async (attr) => { return attr.name; })));
...
async function formatVariantChanges(current, staged, locales) {
    ...
    for (let i = 0; i < Math.max(currentVariants.length, stagedVariants.length); i++) {
        ...
        const sV = stagedVariants.find(async (stagedVariant) => stagedVariant.id === cV?.id);
        if (sV?.id === cV?.id) {
            //Check for SKU Changes
...
async function fetchAddedOrDeletedVariant(currentVariants, stagedVariants) {
    if (currentVariants.length !== stagedVariants.length) {
        let operation = "";
        const variantAdditionDeletionMap = new Map();
        if (currentVariants.length > stagedVariants.length)
            operation = "-";
        else
            operation = "+";
...
async function fetchVariantValueChanges(currentVariant, stagedVariant) {
    //Check for Attribute Value Changes
    const attributeChangeList = [];
    const cvAttributes = currentVariant?.attributes ?? [];
    const svAttributes = stagedVariant?.attributes ?? [];
    const currenVariantAttributeSet = new Set(await Promise.all(cvAttributes?.map(async (attr) => { return attr.name; })));
    const stagedVariantAttributeSet = new Set(await Promise.all(svAttributes.map(async (attr) => { return attr.name; })));
